   ...
   <script type="text/javascript">
	   $(function() {
		 	$("#permissionForm").submit(function() {
		 		var name = $("#permissionForm_name");
		 		var password = $("#permissionForm_password");
		 		var email = $("#permissionForm_email");
		 		var role = $("#permissionForm_role");
		 		var selManager = $("#selManager");
		 		var msg = "";
		 		if ($.trim(name.val()) == ""){
					msg = "账号不能为空！";
					name.focus();
	   	...
	   	function checkCardId(){
			...
			var email = $("#permissionForm_email").val();
			var flag = true;
			$.ajax({
				url:"checkEmailAjax",
				type:"get",
		...
		function assignRole(rid,uname) {
			...
			$.getJSON(url,params,function(data){
				alert(data.msg);
				role = rid;
				$("#managerTD_"+uname).html(data.manager);
			});
		}
					...
					<c:forEach var="user" items="${userList}">
	                    <tr>
	                        ...
	                        <td>
				 				...
				 				<a href="userAction_delete?userId=${user.name}" class="btn btn-danger btn-xs">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<table class="table table-bordered table-striped" width="800px">
						<tr>
							<td>帐号</td>
							<td><input class="form-control" id="permissionForm_name" name="name" placeholder="名称"></td>
						</tr>					
						<tr>
							<td>初始密码</td>
							<td><input class="form-control" type="password" id="permissionForm_password" name="password" placeholder="密码"></td>
						</tr>
						<tr>
						    <td>电子邮箱</td>
						    <td><input class="form-control" id="permissionForm_email" name="email" placeholder="链接">
						    </td>
						</tr>
						<tr>
						    <td>级别</td>
						    <td>
						    	<select class="form-control" id="permissionForm_role" name="role" onchange="getNextManager(this.value)">
